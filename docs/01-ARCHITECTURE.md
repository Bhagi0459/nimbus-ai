...
`WEATHER_API_KEY` and `GROQ_API_KEY`. This is the single reason the backend
exists at all; it does no other business logic beyond "reshape this request,
attach the secret key, forward it, reshape the response."
...
| **Express backend** (`server/index.js`) | A small Node server | Proxies WeatherAPI + Groq, hides both API keys |
...
2. Angular's `WeatherService` calls `GET /api/weather?city=<name>` — note there's
   ...
   need to.
3. The Express backend receives that request, attaches `WEATHER_API_KEY`, and
   calls WeatherAPI's real endpoint on the app's behalf:
...
There's no database anywhere in this project — nothing needs to be remembered
between visits; every search is a fresh live lookup.
